...
export async function scrapeLemFi(fromCur, toCur) {
...
function extractRateFromJSON(body, toCur) {
  ...
  if (Array.isArray(body.rates)) {
    const r = body.rates.find(r => r.currency === toCur || r.currency_code === toCur);
    if (r) {
...
function parseRateFromText(text, fromCur, toCur) {
  const patterns = [
    ...
    new RegExp(`Exchange\\s*[Rr]ate[:\\s]+([0-9][0-9.,]*)`, 'i'),
  ];
...
async function fallbackFromER(fromCur, toCur, multiplier = 0.995) {
  try {
